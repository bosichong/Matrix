...
const App = {
    ...
    methods: {
      ...
      const STRINGHEIGHT = ${this.str_height}; // 字符串高度
      // 字符串列起始坐标
      ...
      // 字体大小
      const MINFONT = ${this.min_font};
      const MAXFONT = ${this.max_font};
      // 字符样色
      const MATRIXCOLOR_R = ${this.matrix_color_r};
      const MATRIXCOLOR_G = ${this.matrix_color_g};
      const MATRIXCOLOR_B = ${this.matrix_color_b};
      // 字符颜色的透明度
      const MINAP = ${this.matrix_color_a_min};
      const MAXAP = ${this.matrix_color_a_max};
      // 下落速度
      const MINSPEED = ${this.min_speed};
      ...
      function getMatrix() {
        // 定义字符串数组
        const mx = {};
        const strings = getStrs(MINLENGTH, MAXLENGTH);
        ...
        let ap = getRandomInt(MINAP, MAXAP); //颜色透明度
        mx.color =
          "rgba(" +
          MATRIXCOLOR_R +
          "," +
          ...
          ap / 10 +
          ")";
        mx.font = getRandomInt(MINFONT, MAXFONT) + "px system-ui";
        mx.strs = strings;
        mx.speed = getRandomInt(MINSPEED, MAXSPEED);
        return mx;
      }
